...
loader.load("./assets/snk.glb",function(gltf){
  ...
  event.preventDefault();
	...
	if (eggposx == root.position.x && eggposy == root.position.z ) {
	...
	for (let i = 0; i < arr.length; i++) {
		if (i==arr.length-1) {
			arr[0].position.x=root.position.x;
			arr[0].position.y=root.position.y;
			arr[0].position.z=root.position.z;
		//console.log(i);
		}
...
loader.load("./assets/plat.glb",function(gltf){
	const platform = gltf.scene;
	platform.scale.set(30,1,30);
